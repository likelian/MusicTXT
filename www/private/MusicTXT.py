...
def loadTitleAndArtist(line):
	words=line.split(':'); title=words[0]; artists=words[1].split(';');
...
def getKeySigniture(line):
	...
	line1=line.replace(" ", "");
	index=line1.find("=");
	if line1[index-1] == "1": isMajor=True;
	else: isMajor=False;
	key=line1[index+1].lower();
	try:
		if line1[index+2] == "#": key += "is"; index += 1;
		if line1[index+2] == "b": key += "es"; index += 1;
		if line1[index+2]=='m': isMajor=False;
	except IndexError:
		pass;
	if isMajor: return "\\key "+key+" \\major";
	else: return "\\key "+key+" \\minor";
#########################################
...
def getTimeSigniture(line):
	...
	if beatValue == 4:
		if beatsNum == 4: stdDur = "1"; stdDurDivided = "2";
		elif beatsNum == 3: stdDur = "2."; stdDurDivided = "4 ";
		elif beatsNum == 2: stdDur = "2"; stdDurDivided = "4";
	elif beatValue == 8:
		if beatsNum == 6: stdDur = "2."; stdDurDivided = "4.";
		elif beatsNum == 3: stdDur = "4."; stdDurDivided = "8 ";
...
def getChords(line):
	...
	if line.find("_") != -1:
		while line.find("__") != -1: line = line.replace("__", "_")
		_idx = [i for i in range(len(line)) if line.find('_', i) == i] #index of _
		DurIdx = []
		...
		for i in DurIdx:
			line = line[:i] + stdDurDivided + line[i+len(stdDur):] #change the duration numbers
		line = line.replace("_", " ")
...
def getPitches(measure):
	...
	measure = measure.translate(table1) #Determine the interval between C and  /do

	global whiteKeyList
	for k in whiteKeyList:
		if key[0] == k:
			keyInterval = whiteKeyList.index(k);
			if keyInterval > 2: keyInterval -= 0.1;
			if key[1:] == "is": keyInterval += 0.1;
			if key[1:] == "es": keyInterval -= 0.1;
	if not isMajor:
		keyInterval += 1.9;
		...
		if keyInterval-1.9 < 6.5 < keyInterval:
			keyInterval += 0.2;
	if keyInterval > 6.5: keyInterval -= 7;
	...
	while measure.find("#") + measure.find("b") != -2:
		for alt in altDict.keys():
			...
			while nextAltIdx != -1:
				...
				if measure[nextAltIdx-1] in ("b", "#", "]"): break;
				elif measure.find(altNote, nextAltIdx, len(measure)) == -1: break;
				else:
	...
	for i, c in enumerate(measure):
		...
		if c.isdigit():
			int_c = int(c);
			int_c += keyInterval;
			if int_c - keyInterval < 3.5 < int_c: int_c += 0.1;
			if int_c - keyInterval < 10.5 < int_c: int_c += 0.1;
			if int_c - keyInterval < 7.5 < int_c: int_c += 0.1;
			...
			if int_c > 7.5: int_c -= 7; Add_Apostrophe = True;
			int_c = round(int_c,1);
	#Convert CMajor-based Jianpu numbers to alphabet-based pitches
			...
			if Add_Accidentals >= 0: c = c + "is"*Add_Accidentals;
			else: c = c + "es"*abs(Add_Accidentals);
			c += "'";
			if Add_Apostrophe: c += "'";
...
def getMeasures(line):
	...
	while True:
		try:
			voiceDict[currentVoice] +="  "+getRhythm(getPitches(getRawMeasure(line, n))) + "\n";
			n += 1;
...
while lineIndex<len(scoreLines):
	line=scoreLines[lineIndex]; lineIndex+=1;
...
if hasChord: file.write(chordCode);
